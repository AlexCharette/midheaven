...
/// whose tags fall outside the chart vocabulary. Returns the accepted
/// excerpts with ids and time anchors assigned; rejects are reported on
/// stderr, preserving the provenance chain.
pub fn verify_gate(
    ...
    for r in raw {
        ...
        let Some(text) = transcript.text.get(s..e) else {
            eprintln!("verify: rejected span {s}..{e} (out of bounds / not a char boundary)");
            continue;
        };
        let bad: Vec<&String> = r.tags.iter().filter(|t| !vocab.contains(*t)).collect();
        if !bad.is_empty() {
            eprintln!("verify: rejected span {s}..{e} (tags outside vocabulary: {bad:?})");
            continue;
        }
        if r.tags.is_empty() {
            continue;
...
mod tests {
    ...
    fn gate_rejects_out_of_vocab_tags() {
        ...
        let ok = verify_gate(&t, raw, &vocab(&["planet:sun"]));
        assert!(ok.is_empty());
    }
    ...
    fn gate_rejects_invalid_spans() {
        ...
        assert!(verify_gate(&t, raw, &vocab(&["planet:sun"])).is_empty());
    }
    ...
    fn gate_output_is_verbatim() {
        ...
        let ok = verify_gate(&t, raw, &vocab(&["planet:moon"]));
        assert_eq!(ok.len(), 1);
